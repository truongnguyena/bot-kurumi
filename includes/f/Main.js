...
try {
    if (appState) {
        ...
        try {
            ...
            appState = appstateUtil.normalizeAppState(appState);
            global.Fca.Data.AppState = appState;
            appstateUtil.applyAppStateToJar(jar, appState);
            ...
            mainPromise = mainPromise
                ...
                .then(function(res){
                    ...
                    if (!Obj || !Obj.ctx || !Obj.api) {
                        const err = new Error('Appstate không hợp lệ hoặc cookie đã hết hạn — hãy export appstate mới từ Facebook.');
                        err.error = err;
                        throw err;
                    }
                    ctx = Obj.ctx;
