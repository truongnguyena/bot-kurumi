...
module.exports = function (defaultFuncs, api, ctx) {
  ...
  function send(form, threadID, messageAndOTID, callback, isGroup) {
    //Full Fix sendMessage - Sửa để nhóm nhỏ 2 người cũng hoạt động
    if (utils.getType(threadID) === "Array") sendContent(form, threadID, false, messageAndOTID, callback);
    else {
      var THREADFIX = "ThreadID".replace("ThreadID", threadID);
      // Nếu threadID ngắn hoặc là user đã biết -> single user
      if (THREADFIX.length <= 15 || global.Fca.isUser.includes(threadID)) {
        sendContent(form, threadID, !isGroup, messageAndOTID, callback);
      }
      // Nếu threadID dài -> LUÔN là group chat (kể cả nhóm 2 người)
      else {
